...
def get_ai_recommendation_with_fallback(
    validation_data: Dict[str, Any], block_hash: str = "", enable_ai: bool = True
) -> str:
    """
    ...
    if enable_ai:
        ...
        if not any(
            error in ai_response.upper()
            for error in ["AI_ERROR", "AI_TIMEOUT", "AI_CALL_ERROR", "AI_UNKNOWN_ERROR"]
        ):
            return ai_response
