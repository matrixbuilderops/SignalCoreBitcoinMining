...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('mining_system.log'),
...
class ProductionMiningLauncher:
    ...
    def validate_environment(self) -> Dict[str, bool]:
        ...
        # Check Bitcoin CLI
        try:
            result = subprocess.run(['bitcoin-cli', '--version'], capture_output=True, timeout=10)
            if result.returncode == 0:
        ...
        while self.running and self.restart_count < self.max_restarts:
            try:
                ...
                if return_code is not None:
                    ...
                    if return_code == 0:
                        ...
                        if self.restart_count < self.max_restarts:
                            ...
                            logger.info(f"Restarting mining system (attempt {self.restart_count}/{self.max_restarts})")
                ...
                if int(uptime) % 300 == 0:  # Every 5 minutes
                    logger.info(f"Mining system running - Uptime: {uptime/3600:.1f} hours, Restarts: {self.restart_count}")
    ...
    def run_production_deployment(self, mode: str = 'production') -> None:
        ...
        logger.info(f"  - ZMQ Monitoring: {'✓' if validation['pyzmq_available'] else '⚠ Fallback'}")
        logger.info(f"  - AI Analysis: {'✓' if validation['ollama_available'] else '⚠ Math Fallback'}")
        logger.info(f"  - Bitcoin RPC: {'✓' if validation['bitcoin_cli_available'] else '⚠ Limited'}")
        ...
        uptime = time.time() - self.start_time
        logger.info(f"Production deployment completed - Total uptime: {uptime/3600:.1f} hours")
