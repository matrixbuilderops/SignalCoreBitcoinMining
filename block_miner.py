...
class BlockMiner:
    ...
    def __init__(self, verbose: bool = True, thinking_mode: bool = False):
        ...
        self.output = OutputHandler(
            OutputMode.THINKING if thinking_mode else 
            OutputMode.VERBOSE if verbose else 
            OutputMode.QUIET
        )
    ...
    def _verify_system_readiness(self) -> bool:
        ...
        # Test model caller
        try:
            test_data = {"level": 16000, "fork_integrity": True}
    ...
    def _process_mining_opportunity(self, block_data: bytes, block_hash: str) -> None:
        ...
        try:
            ...
            analysis = self.model_caller.analyze_mining_opportunity(validation_results, block_hash)
    ...
    def _attempt_submission(self, validation_results: Dict[str, Any], 
                          analysis: Dict[str, Any], block_hash: str) -> None:
        """
        ...
        try:
            ...
            # Submit solution
            submitted_hash = self.submission_client.submit_block_solution(validation_results)
            ...
            if submitted_hash:
                ...
                self.output.log_mining_result(False, block_hash, "Submission validation failed")
    ...
    def _log_progress(self) -> None:
        ...
        if runtime > 0:
            ...
            success_rate = (self.stats.successful_submissions / self.stats.blocks_processed * 100) if self.stats.blocks_processed > 0 else 0
    ...
    def _show_final_stats(self) -> None:
        ...
        if self.stats.blocks_processed > 0:
            success_rate = (self.stats.successful_submissions / self.stats.blocks_processed * 100)
    ...
    def get_mining_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate_percent": (self.stats.successful_submissions / self.stats.blocks_processed * 100) if self.stats.blocks_processed > 0 else 0,
            "blocks_per_hour": (self.stats.blocks_processed / runtime * 3600) if runtime > 0 else 0,
