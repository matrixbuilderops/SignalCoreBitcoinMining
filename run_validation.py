...
def main():
    ...
    success, stdout, stderr = run_command(
        ["flake8", "*.py", "--max-line-length=88"], "Flake8"
    )
    ...
    success, stdout, stderr = run_command(
        ["mypy", "*.py", "--ignore-missing-imports"], "MyPy"
    )
    ...
    success, stdout, stderr = run_command(
        ["bandit", "-r", ".", "--quiet", "--format", "txt"], "Bandit"
    )
    ...
    run_command(
        ["coverage", "run", "--source=.", "test_mining_engine.py"], "Coverage run"
    )
    ...
    print(
        f"   {'✅' if success else '❌'} Interrogate: {'PASSED' if success else 'FAILED'}"
    )

    # 9. Mathematical validation with Z3 and hypothesis
    print("🧮 Running advanced mathematical validation...")
    success, stdout, stderr = run_command(
        ["python", "test_advanced_validation.py"], "Z3 & Hypothesis"
    )
    validations.append(("Z3/Hypothesis", success))
    print(
        f"   {'✅' if success else '❌'} Z3/Hypothesis: {'PASSED' if success else 'FAILED'}"
    )

    # 10. Core functionality tests
    print("🧪 Running core functionality tests...")
    success, stdout, stderr = run_command(
        ["python", "test_mining_engine.py"], "Core tests"
    )
    validations.append(("Core Tests", success))
    print(
        f"   {'✅' if success else '❌'} Core Tests: {'PASSED' if success else 'FAILED'}"
    )
