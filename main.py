...
Requirements:
  ...
  - Wallet: SignalCoreBitcoinMining
    ...
    parser.add_argument(
        "--test", 
        action="store_true", 
        help="Run in test mode with mock blockchain data"
    )
    
    parser.add_argument(
        "--validate", 
        action="store_true", 
        help="Run system validation checks and exit"
    )
    ...
    try:
        miner = BlockMiner(
            verbose=not args.quiet and not args.thinking,
        ...
        if args.validate:
            ...
            print("Note: Will show AI/RPC errors in test environment - this is expected")
            print("-" * 50)
